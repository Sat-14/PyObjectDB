...
class DemoStorage(ConflictResolvingStorage):
    ...
    def __init__(self, name=None, base=None, changes=None,
        ...
        :class:`~PyObjectDB.MappingStorage.MappingStorage` will be
        ...
        if changes is None:
            ...
            changes = PyObjectDB.MappingStorage.MappingStorage()
            zope.interface.alsoProvides(self, PyObjectDB.interfaces.IBlobStorage)
            if close_changes_on_close is None:
                close_changes_on_close = False
        else:
            if PyObjectDB.interfaces.IBlobStorage.providedBy(changes):
                zope.interface.alsoProvides(self, PyObjectDB.interfaces.IBlobStorage)
            if close_changes_on_close is None:
    ...
    def _blobify(self):
        if (self._temporary_changes and
                isinstance(self.changes, PyObjectDB.MappingStorage.MappingStorage)):
            blob_dir = tempfile.mkdtemp('.demoblobs')
            _temporary_blobdirs[
                weakref.ref(self, cleanup_temporary_blobdir)
            ] = blob_dir
            self.changes = PyObjectDB.blob.BlobStorage(blob_dir, self.changes)
    ...
    def _copy_methods_from_changes(self, changes):
        ...
        if supportsUndo is not None and supportsUndo():
            for meth in ('supportsUndo', 'undo', 'undoLog', 'undoInfo'):
                setattr(self, meth, getattr(changes, meth))
            zope.interface.alsoProvides(self, PyObjectDB.interfaces.IStorageUndoable)
    ...
    def getTid(self, oid):
        ...
        except PyObjectDB.POSException.POSKeyError:
    ...
    def loadBlob(self, oid, serial):
        try:
            ...
            try:
                ...
                if not PyObjectDB.interfaces.IBlobStorage.providedBy(self.base):
    ...
    def new_oid(self):
        with self._lock:
            while 1:
                oid = PyObjectDB.utils.p64(self._next_oid)
...
def cleanup_temporary_blobdir(
    ...
    if blob_dir and os.path.exists(blob_dir):
        PyObjectDB.blob.remove_committed_dir(blob_dir)
