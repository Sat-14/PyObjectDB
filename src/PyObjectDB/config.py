...
db_schema_path = os.path.join(PyObjectDB.__path__[0], "config.xml")
_db_schema = None

s_schema_path = os.path.join(PyObjectDB.__path__[0], "storage.xml")
_s_schema = None
...
def databaseFromString(s):
    ...
    The string must contain one or more :ref:`PyObjectDB
    <database-text-configuration>` sections.

    The database defined by the first section is returned.

    If :ref:`more than one PyObjectDB section is provided
...
class PyObjectDBDatabase(BaseConfig):
...
class BlobStorage(BaseConfig):
    ...
    def open(self):
        from PyObjectDB.blob import BlobStorage
