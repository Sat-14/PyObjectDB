...
class IConnection(Interface):
    ...
    Typical use of PyObjectDB is for each thread to have its own
    ...
    def sync():
        ...
        This method was especially useful in PyObjectDB 3.2 to better support
...
class IStorage(Interface):
    ...
    to the object versions written by the transactions.  PyObjectDB relies
    ...
    working of PyObjectDB that the resulting tids are increasing with
...
class IMultiCommitStorage(IStorage):
    ...
    It's likely that future versions of PyObjectDB will require all storages
...
class IMVCCStorage(IStorage):
    ...
    MVCC semantics at the PyObjectDB storage layer. When PyObjectDB.Connection uses
    ...
    FileStorage, rely on PyObjectDB.Connection to provide MVCC semantics, so
    in that case, one storage instance is shared by many
    PyObjectDB.Connections. Applications that use PyObjectDB.Connection always have
    a snapshot view of the database; IMVCCStorage only modifies which
    layer of PyObjectDB provides MVCC.

    Furthermore, IMVCCStorage changes the way object invalidation
    works. An essential feature of PyObjectDB is the propagation of object
    ...
    PyObjectDB.Connection to poll for a list of invalidated objects.
    ...
    def poll_invalidations():
        ...
        removed from the PyObjectDB in-memory cache.
        ...
        connection has already been packed. In that case, the PyObjectDB in-memory
...
class IBlob(Interface):
    """A BLOB supports efficient handling of large data within PyObjectDB."""
...
class IBroken(Interface):
    ...
    def __setattr__(name, value):
        ...
        PyObjectDB.broken.BrokenModified exception.
        """
