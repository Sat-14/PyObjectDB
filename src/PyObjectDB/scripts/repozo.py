...
Where:
    ...
    -B / --backup
        Backup current PyObjectDB file.

    -R / --recover
        Restore a PyObjectDB file from a backup.
...
Options for -R/--recover:
    ...
    -o filename
    --output=filename
        Write recovered PyObjectDB to given file.  By default, the file is
        written to stdout.
    ...
    --with-verify
        ...
        check fails, the partially recovered PyObjectDB will be left in place.
...
def do_recover(options):
    ...
    if options.output is None:
        ...
        # Delete old PyObjectDB before recovering backup as size of
        # old PyObjectDB + full partial file may be superior to free disk space
    ...
    if options.output is not None:
        try:
            ...
            log("PyObjectDB has been fully recovered as %s, but it cannot be renamed"
