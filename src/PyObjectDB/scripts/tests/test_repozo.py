...
import PyObjectDB.tests.util  # layer used at class scope
...
class Test_parseargs(unittest.TestCase):
    ...
    def setUp(self):
        from PyObjectDB.scripts import repozo
...
class FileopsBase:
    ...
    def _makeChunks(self):
        from PyObjectDB.scripts.repozo import READCHUNK
...
class Test_dofile(unittest.TestCase, FileopsBase):
    ...
    def _callFUT(self, func, fp, n):
        from PyObjectDB.scripts.repozo import dofile
...
class Test_checksum(unittest.TestCase, FileopsBase):
    ...
    def _callFUT(self, fp, n):
        from PyObjectDB.scripts.repozo import checksum
...
class Test_copyfile(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, options, dest, start, n):
        from PyObjectDB.scripts.repozo import copyfile
    ...
    def test_w_gzip(self):
        from PyObjectDB.scripts.repozo import _GzipCloser
...
class Test_concat(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, files, ofp):
        from PyObjectDB.scripts.repozo import concat
    ...
    def _makeFile(self, name, text, gzip_file=False):
        ...
        if self._repository_directory is None:
            self._repository_directory = tempfile.mkdtemp(prefix='PyObjectDB-test-')
...
class Test_gen_filename(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, options, ext=_marker):
        from PyObjectDB.scripts.repozo import gen_filename
        if ext is _marker:
...
class Test_find_files(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, options):
        from PyObjectDB.scripts.repozo import find_files
...
class Test_scandat(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, repofiles):
        from PyObjectDB.scripts.repozo import scandat
...
class Test_delete_old_backups(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, options=None, filenames=()):
        from PyObjectDB.scripts.repozo import delete_old_backups
        if options is None:
...
class Test_do_full_backup(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, options):
        from PyObjectDB.scripts.repozo import do_full_backup
        return do_full_backup(options)

    def _makeDB(self):
        import tempfile
        self._data_directory = tempfile.mkdtemp(prefix='PyObjectDB-test-')
        return OurDB(self._data_directory)

    def test_dont_overwrite_existing_file(self):
        from PyObjectDB.scripts.repozo import WouldOverwriteFiles
        from PyObjectDB.scripts.repozo import gen_filename
        db = self._makeDB()
...
class Test_do_incremental_backup(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, options, reposz, repofiles):
        from PyObjectDB.scripts.repozo import do_incremental_backup
...
class Test_do_recover(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, options):
        from PyObjectDB.scripts.repozo import do_recover
    ...
    def test_no_files(self):
        from PyObjectDB.scripts.repozo import NoFiles
        options = self._makeOptions(date=None,
    ...
    def test_w_full_backup_latest_no_index(self):
        import tempfile
        dd = self._data_directory = tempfile.mkdtemp(prefix='PyObjectDB-test-')
    ...
    def test_w_incr_backup_with_verify_sum_inconsistent(self):
        ...
        from PyObjectDB.scripts.repozo import VerificationFail
        dd = self._data_directory = tempfile.mkdtemp(prefix='PyObjectDB-test-')
        output = os.path.join(dd, 'Data.fs')
        options = self._makeOptions(date='2010-05-15-13-30-57',
                                    output=output,
...
class Test_do_verify(OptionsTestBase, unittest.TestCase):
    ...
    def _callFUT(self, options):
        from PyObjectDB.scripts.repozo import do_verify
        do_verify(options)
...
class MonteCarloTests(unittest.TestCase):
    ...
    layer = PyObjectDB.tests.util.MininalTestLayer('repozo')

    def setUp(self):
        # compute directory names
        import tempfile
        self.basedir = tempfile.mkdtemp(prefix='PyObjectDB-test-')
    ...
    def _callRepozoMain(self, argv):
        from PyObjectDB.scripts.repozo import main
        main(argv)

    @PyObjectDB.tests.util.time_monotonically_increases
