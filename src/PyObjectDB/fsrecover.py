...
Important:  The PyObjectDB package must be importable.  You may need to adjust
...
from PyObjectDB.FileStorage import TransactionRecord
...
def recover(inp, outp, verbose=0, partial=False, force=False, pack=None):
    ...
    f = open(inp, "rb")
    if f.read(4) != PyObjectDB.FileStorage.packed_version:
        die("input is not a file storage")

    f.seek(0, 2)
    file_size = f.tell()

    ofs = PyObjectDB.FileStorage.FileStorage(outp, create=1)
    _ts = None
    ok = 1
    prog1 = 0
    undone = 0
