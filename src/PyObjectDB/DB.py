...
import PyObjectDB.serialize
from PyObjectDB import utils
from PyObjectDB import valuedoc
from PyObjectDB.broken import find_global
from PyObjectDB.Connection import Connection
from PyObjectDB.Connection import TransactionMetaData
from PyObjectDB.Connection import noop
from PyObjectDB.interfaces import IDatabase
from PyObjectDB.interfaces import IMVCCStorage
from PyObjectDB.utils import z64


logger = logging.getLogger('PyObjectDB.DB')
...
class DB:
    ...
    #: Database storage, implementing :interface:`~PyObjectDB.interfaces.IStorage`
    ...
    def __init__(self,
        ...
        :param storage: the storage used by the database, such as a
             :class:`~PyObjectDB.FileStorage.FileStorage.FileStorage`.
             This can be a string path name to use a constructed
             :class:`~PyObjectDB.FileStorage.FileStorage.FileStorage`
             storage or ``None`` to use a constructed
             :class:`~PyObjectDB.MappingStorage.MappingStorage`.
        ...
        :param callable class_factory: A callable
            ...
            ``DB.classFactory`` method is used; it wraps
             :func:`PyObjectDB.broken.find_global` to provide this
        ...
        if isinstance(storage, str):
            from PyObjectDB import FileStorage  # noqa: F401 import unused
            storage = PyObjectDB.FileStorage.FileStorage(storage, **storage_args)
        elif storage is None:
            from PyObjectDB import MappingStorage  # noqa: F401 import unused
            storage = PyObjectDB.MappingStorage.MappingStorage(**storage_args)
        else:
        ...
        self.references = PyObjectDB.serialize.referencesf
    ...
    def cacheExtremeDetail(self):
        ...
        excludes references help by PyObjectDB itself.
        """
        detail = []
    ...
    def connectionDebugInfo(self):
        ...
        calling :meth:`~PyObjectDB.Connection.Connection.getDebugInfo` on
    ...
    def history(self, oid, size=1):
        ...
        See :meth:`PyObjectDB.interfaces.IStorage.history`.
        """
    ...
    def undoLog(self, *args, **kw):
        ...
        See :meth:`PyObjectDB.interfaces.IStorageUndoable.undoLog`.
        """
    ...
    def undoInfo(self, *args, **kw):
        ...
        See :meth:`PyObjectDB.interfaces.IStorageUndoable.undoInfo`.
        """
        if not self.supportsUndo():
            return ()
...
def connection(*args, **kw):
    """Create a database :class:`connection <PyObjectDB.Connection.Connection>`.
