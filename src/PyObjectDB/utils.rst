=====================
PyObjectDB Utilities Module
=====================

The PyObjectDB.utils module provides a number of helpful, somewhat random
:), utility functions.
...
PyObjectDB uses 64-bit transaction ids that are typically represented as
...
Functions p64 and u64 pack and unpack integers as strings:
    ...
    >>> PyObjectDB.utils.p64(250347764455111456)
    b'\x03yi\xf7"\xa8\xfb '

    >>> print(PyObjectDB.utils.u64(b'\x03yi\xf7"\xa8\xfb '))
    250347764455111456

The constant z64 has zero packed as a 64-bit string:

    >>> PyObjectDB.utils.z64
...
time:
    ...
    >>> tid = PyObjectDB.utils.newTid(None)
    >>> tid
...
it to a time stamp.
    ...
    >>> print(PyObjectDB.TimeStamp.TimeStamp(tid))
...
pass an existing tid.  Let's pass the tid we just got.
    ...
    >>> tid2 = PyObjectDB.utils.newTid(tid)
    >>> PyObjectDB.utils.u64(tid), PyObjectDB.utils.u64(tid2)
...
time, the time stamp we get will be based on the time:
    ...
    >>> tid = PyObjectDB.utils.newTid(tid2)
    >>> print(PyObjectDB.TimeStamp.TimeStamp(tid))
...
.. Edge cases
   ...
   We can get the method from the class:
    ...
    <PyObjectDB.utils.Locked object at ...>
...
supports optional method preconditions [1]_.
    ...
    ...
    ...     @PyObjectDB.utils.locked(opened, not_in_transaction)
    ...     def meth(self, *args, **kw):
    ...         print('meth %r %r' % (args, kw))
...
.. [1] Arguably, preconditions should be handled via separate
   descriptors, but for PyObjectDB storages, almost all methods need to be
