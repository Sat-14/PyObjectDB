...
integrity away from the PyObjectDB, and puts it in the hands of the developer
writing the conflict resolution code.

The current conflict resolution code is implemented with a storage mix-in
found in PyObjectDB/ConflictResolution.py.  The idea's proposal, and an explanation
of the interface, can be found here:
http://www.zope.org/Members/jim/PyObjectDB/ApplicationLevelConflictResolution
...
for change (as of this writing, according to Jim Fulton, the PyObjectDB
...
And now we will make a conflict.
    ...
    ...
    PyObjectDB.POSException.ConflictError: database conflict error...
...
in the state dictionary are PyObjectDB.ConflictResolution.PersistentReference
...
after PyObjectDB 3.8 and addresses a serious problem for when persistent
objects are compared in an _p_resolveConflict, such as that in the PyObjectDB
...
.. [#cross-database] We need a whole different set of databases for this.
    ...
    >>> databases = {}
    >>> db1 = PyObjectDB.DB('1', databases=databases, database_name='1')
    >>> db2 = PyObjectDB.DB('2', databases=databases, database_name='2')
...
.. [#instantiation_test] We'll simply instantiate PersistentReferences
    with examples of types described in PyObjectDB/serialize.py.

    >>> from PyObjectDB.ConflictResolution import PersistentReference
