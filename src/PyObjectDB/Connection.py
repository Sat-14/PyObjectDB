...
from PyObjectDB import utils
from PyObjectDB.blob import SAVEPOINT_SUFFIX
from PyObjectDB.blob import Blob
from PyObjectDB.blob import remove_committed_dir
from PyObjectDB.blob import rename_or_copy_blob
from PyObjectDB.ExportImport import ExportImport
from PyObjectDB.interfaces import IBlobStorage
from PyObjectDB.interfaces import IConnection
from PyObjectDB.interfaces import IStorageTransactionMetaData
from PyObjectDB.POSException import ConflictError
from PyObjectDB.POSException import ConnectionStateError
from PyObjectDB.POSException import InvalidObjectReference
from PyObjectDB.POSException import ReadConflictError
from PyObjectDB.POSException import ReadOnlyHistoryError
from PyObjectDB.POSException import Unsupported
from PyObjectDB.serialize import ObjectReader
from PyObjectDB.serialize import ObjectWriter
from PyObjectDB.utils import oid_repr
...
def resetCaches():
    ...
    To use the new code immediately, the refresh feature asks PyObjectDB to
...
class Connection(ExportImport):
    """Connection to PyObjectDB for loading and storing objects.

    Connections manage object state in collaboration with transaction
    managers.  They're created by calling the
    :meth:`~PyObjectDB.DB.open` method on :py:class:`database
    <PyObjectDB.DB>` objects.
    """
    ...
    ##########################################################################
    # Connection methods, PyObjectDB.IConnection

    def __init__(self, db, cache_size=400, before=None, cache_size_bytes=0):
        """Create a new Connection."""

        self._log = logging.getLogger('PyObjectDB.Connection')
        self._debug_info = ()
    ...
    def close(self, primary=True):
        ...
        if self.opened and self.transaction_manager is not None:
            ...
            # __onCloseCallbacks closed the DB already, .e.g, PyObjectDB.connection()
            # does this.
    ...
    ##########################################################################
    # PROTECTED stuff (used by e.g. PyObjectDB.DB.DB)
...
class TmpStore:
    ...
    def openCommittedBlobFile(self, oid, serial, blob=None):
        ...
        if blob is None:
            ...
            return PyObjectDB.blob.BlobFile(blob_filename, 'r', blob)
...
large_record_size option of the PyObjectDB.DB constructor (or the
