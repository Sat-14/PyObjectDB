...
def pack_with_repeated_blob_records():
    ...
    >>> fs = PyObjectDB.FileStorage.FileStorage('t', blob_dir='bobs')
    >>> db = PyObjectDB.DB(fs)
    >>> conn = db.open()
    >>> conn.root()[1] = PyObjectDB.blob.Blob()
    >>> transaction.commit()
...
_save_index can fail for large indexes.
    ...
    >>> for i in range(5000):
    ...     oid += (1<<16)
    ...     fs.store(PyObjectDB.utils.p64(oid), PyObjectDB.utils.z64, b'x', '', t)
    >>> _ = fs.tpc_vote(t)
    >>> _ = fs.tpc_finish(t)
...
patch `copyToPacktime` to fail
    ...
    >>> from PyObjectDB.FileStorage import fspack
