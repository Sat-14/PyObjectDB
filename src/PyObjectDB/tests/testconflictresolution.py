...
import PyObjectDB.ConflictResolution
...
def setUp(test):
    ...
    PyObjectDB.ConflictResolution._class_cache.clear()
    PyObjectDB.ConflictResolution._unresolvable.clear()
...
def succeed_with_resolution_when_state_is_unchanged():
    ...
    >>> db = PyObjectDB.DB('t.fs') # FileStorage!
    >>> storage = db.storage
    >>> conn = db.open()
...
the original:
    ...
    ...
    PyObjectDB.POSException.ConflictError: database conflict error (oid 0x01, ...
...
mechanism.
    ...
    Traceback (most recent call last):
    ...
    PyObjectDB.POSException.ConflictError: database conflict error (oid 0x02, ...
...
Bwahaha:
    ...
    Traceback (most recent call last):
    ...
    PyObjectDB.POSException.ConflictError: database conflict error (oid ...
...
def resolve_even_when_xdb_referenced_classes_are_absent():
    ...
    >>> databases = {}
    >>> db = PyObjectDB.DB('t.fs', databases=databases, database_name='')
    >>> db2 = PyObjectDB.DB('o.fs', databases=databases, database_name='o')
    >>> storage = db.storage
    >>> conn = db.open()
    >>> conn.root.x = Resolveable()
    >>> transaction.commit()
...
Install a log handler to be able to show log entries
    ...
    >>> handler = InstalledHandler('PyObjectDB.ConflictResolution',
    ...     level=logging.DEBUG)
...
Let's see what went into the log:
    ...
    PyObjectDB.ConflictResolution ERROR Unexpected error while trying to resolve conflict on <class 'PyObjectDB.tests.testconflictresolution.FailHard'>

    >>> msg = handler.records[1]
    >>> print(msg.name, msg.levelname, msg.getMessage())
    PyObjectDB.ConflictResolution DEBUG Conflict resolution on <class 'PyObjectDB.tests.testconflictresolution.Unresolvable'> failed with ConflictError: database conflict error
...
def test_suite():
    return unittest.TestSuite([
        manuel.testing.TestSuite(
            manuel.doctest.Manuel(checker=PyObjectDB.tests.util.checker)
