...
class DBTests(PyObjectDB.tests.util.TestCase):

    def setUp(self):
        PyObjectDB.tests.util.TestCase.setUp(self)
        self.db = PyObjectDB.DB('test.fs')
    ...
    def test_references(self):
        ...
        self.assertIs(self.db.references, PyObjectDB.serialize.referencesf)
...
class TransactionalUndoTests(unittest.TestCase):
    ...
    def _makeOne(self):
        from PyObjectDB.DB import TransactionalUndo
...
def passing_a_file_name_to_DB():
    ...
    (Also note that we can access DB in PyObjectDB.)

    >>> import os
    >>> db = PyObjectDB.DB('data.fs')
    >>> db.storage # doctest: +ELLIPSIS
    <PyObjectDB.FileStorage.FileStorage.FileStorage object at ...
...
def passing_None_to_DB():
    ...
    >>> db.storage # doctest: +ELLIPSIS
    <PyObjectDB.MappingStorage.MappingStorage object at ...
    >>> db.close()
    """
...
def class_factory_parameter():
    ...
    >>> db = PyObjectDB.DB(None, class_factory=my_factory)
...
def open_convenience():
    ...
    >>> conn = PyObjectDB.connection('data.fs')
    >>> conn.root()
    {}
    ...
    >>> conn = PyObjectDB.connection('data.fs', blob_dir='blobs')
    >>> conn.root()['b'] = PyObjectDB.blob.Blob(b'test')
    >>> transaction.commit()
    >>> conn.close()
...
def connection_allows_empty_version_for_idiots():
    r"""
    >>> db = PyObjectDB.DB('t.fs')
    >>> c = PyObjectDB.tests.util.assert_deprecated(
    ...       (lambda : db.open('')),
...
users from shooting themselves in the foot.
    ...
    >>> db = PyObjectDB.DB('t.fs', create=True)
    >>> conn = db.open()
    >>> conn.root.x = 'x'*(1<<24)
    >>> PyObjectDB.tests.util.assert_warning(UserWarning, transaction.commit,
    ...    "object you're saving is large.")
    >>> db.close()

The large_record_size option can be used to control the record size:

    >>> db = PyObjectDB.DB('t.fs', create=True, large_record_size=999)
    >>> conn = db.open()
    >>> conn.root.x = 'x'
    >>> transaction.commit()
...
def minimally_test_connection_timeout():
    ...
    >>> db = PyObjectDB.DB(None, pool_timeout=.01)
    >>> c1 = db.open()
...
def test_suite():
    ...
    s.addTest(doctest.DocTestSuite(
        setUp=PyObjectDB.tests.util.setUp, tearDown=PyObjectDB.tests.util.tearDown,
