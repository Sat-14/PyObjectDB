...
#
##############################################################################
"""PyObjectDB-based (functional) tests persistent weakrefs
"""
...
def test_weakrefs_functional():
    ...
    >>> import PyObjectDB.tests.MinPO
    >>> import PyObjectDB.tests.util
    >>> ob = PyObjectDB.tests.MinPO.MinPO()
    >>> ref = WeakRef(ob)
    >>> ref() is ob
    True
    ...
    >>> ob2 = PyObjectDB.tests.MinPO.MinPO(1)
    >>> WeakRef(ob2) == ref
    False
    ...
    >>> dbA = PyObjectDB.tests.util.DB(
    ...     database_name = 'dbA',
    ...     )
    >>> dbB = PyObjectDB.tests.util.DB(
    ...     database_name = 'dbB',
...
def test_PersistentWeakKeyDictionary():
    ...
    >>> import PyObjectDB.tests.util
    >>> p1 = PyObjectDB.tests.util.P('p1')
    >>> p2 = PyObjectDB.tests.util.P('p2')
    >>> p3 = PyObjectDB.tests.util.P('p3')
    >>> d[p1] = 1
    >>> d[p2] = 2
    >>> d[p3] = 3
...
def test_PersistentWeakKeyDictionary_get():
    ...
    >>> import PyObjectDB.tests.util
    >>> from persistent.wref import PersistentWeakKeyDictionary
    >>> key = PyObjectDB.tests.util.P("key")
    >>> missing = PyObjectDB.tests.util.P("missing")
