...
import PyObjectDB.FileStorage
import PyObjectDB.tests.hexstorage
import PyObjectDB.tests.testblob
from PyObjectDB import DB
from PyObjectDB import POSException
from PyObjectDB._compat import _protocol
from PyObjectDB._compat import dump
from PyObjectDB._compat import dumps
from PyObjectDB.Connection import TransactionMetaData
from PyObjectDB.fsIndex import fsIndex
from PyObjectDB.interfaces import IStorageWrapper
from PyObjectDB.tests import BasicStorage
from PyObjectDB.tests import ConflictResolution
from PyObjectDB.tests import Corruption
...
from PyObjectDB.tests import PersistentStorage
from PyObjectDB.tests import ReadOnlyStorage
from PyObjectDB.tests import RecoveryStorage
...
from PyObjectDB.tests import TransactionalUndoStorage
from PyObjectDB.tests.StorageTestBase import MinPO
from PyObjectDB.tests.StorageTestBase import PyObjectDB_pickle
from PyObjectDB.utils import U64
from PyObjectDB.utils import load_current
...
class FileStorageTests(
    ...
    def open(self, **kwargs):
        self._storage = PyObjectDB.FileStorage.FileStorage('FileStorageTests.fs',
                                                     **kwargs)
    ...
    def testStoreBumpsOid(self):
        ...
        # Before PyObjectDB 3.2.6, this failed, with ._oid == z64.
    ...
    def testCorruptionInPack(self):
        ...
        # PyObjectDB blew up if the .fs file had such damage:  it detected the
        # damage, but the code to raise CorruptedError referenced an undefined
        # global.
        import time

        from PyObjectDB.FileStorage.format import CorruptedError
        from PyObjectDB.serialize import referencesf
...
class FileStorageHexTests(FileStorageTests):
    ...
    def open(self, **kwargs):
        self._storage = PyObjectDB.tests.hexstorage.HexStorage(
            PyObjectDB.FileStorage.FileStorage('FileStorageTests.fs', **kwargs))
...
class FileStorageHexTestsWithBlobsEnabled(FileStorageTests):
    ...
    def open(self, **kwargs):
        if 'blob_dir' not in kwargs:
            ...
            kwargs['blob_dir'] = 'blobs'
        FileStorageTests.open(self, **kwargs)
        self._storage = PyObjectDB.tests.hexstorage.HexStorage(self._storage)
...
class FileStorageRecoveryTest(
    ...
    def setUp(self):
        StorageTestBase.StorageTestBase.setUp(self)
        self._storage = PyObjectDB.FileStorage.FileStorage("Source.fs", create=True)
        self._dst = PyObjectDB.FileStorage.FileStorage("Dest.fs", create=True)
    ...
    def new_dest(self):
        return PyObjectDB.FileStorage.FileStorage('Dest.fs')
...
class FileStorageHexRecoveryTest(FileStorageRecoveryTest):
    ...
    def setUp(self):
        StorageTestBase.StorageTestBase.setUp(self)
        self._storage = PyObjectDB.tests.hexstorage.HexStorage(
            PyObjectDB.FileStorage.FileStorage("Source.fs", create=True))
        self._dst = PyObjectDB.tests.hexstorage.HexStorage(
            PyObjectDB.FileStorage.FileStorage("Dest.fs", create=True))


class FileStorageNoRestore(PyObjectDB.FileStorage.FileStorage):
...
class AnalyzeDotPyTest(StorageTestBase.StorageTestBase):
    ...
    def testanalyze(self):
        ...
        from PyObjectDB.scripts import analyze
        ...
        for i in range(2):
            ...
            store(2, PyObjectDB_pickle(MinPO(2)))
        ...
        # from PyObjectDB.scripts.analyze.analyze
        fsi = self._storage.iterator()
        rep = analyze.Report()
        for txn in fsi:
            analyze.analyze_trans(rep, txn)

        # from PyObjectDB.scripts.analyze.report
...
def testTimeTravelOnOpen():
    ...
    >>> handler = InstalledHandler('PyObjectDB.FileStorage')
...
track of the transactions along the way:
    ...
    >>> fs = PyObjectDB.FileStorage.FileStorage('t.fs', create=True)
    >>> db = DB(fs)
    >>> conn = db.open()
...
def deal_with_finish_failures():
    ...
    >>> fs = PyObjectDB.FileStorage.FileStorage('data.fs')
    >>> db = DB(fs)
    >>> conn = db.open()
    >>> conn.root()[1] = 1
    >>> transaction.commit()
    ...
    >>> print(handler)
    PyObjectDB.FileStorage CRITICAL
...
def pack_with_open_blob_files():
    ...
    >>> fs = PyObjectDB.FileStorage.FileStorage('data.fs', blob_dir='blobs')
    >>> db = PyObjectDB.DB(fs)
    >>> tm1 = transaction.TransactionManager()
    >>> conn1 = db.open(tm1)
    >>> import PyObjectDB.blob
    >>> conn1.root()[1] = PyObjectDB.blob.Blob()
    ...
    >>> conn2 = db.open(tm2)
    >>> f = conn1.root()[1].open()
    >>> conn1.root()[2] = PyObjectDB.blob.Blob()
...
def readonly_open_nonexistent_file():
    ...
    >>> try:
    ...     fs = PyObjectDB.FileStorage.FileStorage('nonexistent.fs', read_only=True)
...
def test_suite():
    ...
    suite.addTest(PyObjectDB.tests.testblob.storage_reusable_suite(
        'BlobFileStorage',
        lambda name, blob_dir:
        PyObjectDB.FileStorage.FileStorage('%s.fs' % name, blob_dir=blob_dir),
    ...
    suite.addTest(PyObjectDB.tests.testblob.storage_reusable_suite(
        'BlobFileHexStorage',
        lambda name, blob_dir:
        PyObjectDB.tests.hexstorage.HexStorage(
            PyObjectDB.FileStorage.FileStorage('%s.fs' % name, blob_dir=blob_dir)),
        test_blob_storage_recovery=True,
        test_packing=True,
    ))
    suite.addTest(PackableStorage.IExternalGC_suite(
        lambda: PyObjectDB.FileStorage.FileStorage(
