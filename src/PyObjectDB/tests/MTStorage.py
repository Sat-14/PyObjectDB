...
class PyObjectDBClientThread(TestThread):
...
class StorageClientThread(TestThread):
    ...
    def dostore(self, i):
        data = PyObjectDB_pickle(MinPO((self.name, i)))
...
class MTStorage:
    ...
    def test2PyObjectDBThreads(self):
        db = PyObjectDB.DB(self._storage)
        self._checkNThreads(2, PyObjectDBClientThread, db, self)
        db.close()

    def test7PyObjectDBThreads(self):
        db = PyObjectDB.DB(self._storage)
        self._checkNThreads(7, PyObjectDBClientThread, db, self)
        db.close()
