...
from PyObjectDB.config import databaseFromString
...
class ConnectionDotAdd(PyObjectDB.tests.util.TestCase):

    def setUp(self):
        PyObjectDB.tests.util.TestCase.setUp(self)
        from PyObjectDB.Connection import Connection
    ...
    def test_add(self):
        from PyObjectDB.POSException import InvalidObjectReference
        obj = StubObject()
    ...
    def test__resetCacheResetsReader(self):
        # https://bugs.launchpad.net/PyObjectDB/+bug/142667
...
class SetstateErrorLoggingTests(PyObjectDB.tests.util.TestCase):

    def setUp(self):
        PyObjectDB.tests.util.TestCase.setUp(self)
        self.db = databaseFromString("<PyObjectDB>\n<mappingstorage/>\n</PyObjectDB>")
        self.datamgr = self.db.open()
        self.object = StubObject()
        self.datamgr.add(self.object)
        transaction.commit()
        self.handler = loggingsupport.InstalledHandler("PyObjectDB")
    ...
    def test_closed_connection_wont_setstate(self):
        ...
        self.datamgr.close()
        self.assertRaises(
            PyObjectDB.POSException.ConnectionStateError,
            self.datamgr.setstate, self.object)
        record, = self.handler.records
        self.assertEqual(
            record.msg,
            "Shouldn't load state for PyObjectDB.tests.testConnection.StubObject"
...
class UserMethodTests(unittest.TestCase):
    ...
    def doctest_root(self):
        ...
        >>> db = databaseFromString("<PyObjectDB>\n<mappingstorage/>\n</PyObjectDB>")
        >>> cn = db.open()
        >>> root = cn.root()
        >>> type(root).__name__
        'PersistentMapping'
    ...
    def doctest_get(self):
        ...
        Traceback (most recent call last):
          ...
        PyObjectDB.POSException.POSKeyError: 0x01
        """
    ...
    def doctest_close(self):
        ...
        Traceback (most recent call last):
          ...
        PyObjectDB.POSException.ConnectionStateError: The database connection is closed
        >>> p = Persistent()
        >>> cn.add(p)
        Traceback (most recent call last):
          ...
    ...
    def doctest_close_with_pending_changes(self):
        ...
        Traceback (most recent call last):
          ...
        PyObjectDB.POSException.ConnectionStateError: Cannot close a connection joined to a transaction
    ...
    def doctest_close_dispatches_to_activity_monitors(self):
        ...
        Set up a multi-database:
            ...
            >>> db1 = PyObjectDB.DB(None)
            >>> db2 = PyObjectDB.DB(None, databases=db1.databases, database_name='2',
            ...               cache_size=10)
            >>> conn1 = db1.open()
        ...
        Add activity monitors to both dbs:
            ...
            >>> from PyObjectDB.ActivityMonitor import ActivityMonitor
...
class InvalidationTests(unittest.TestCase):
    ...
    # on the various concurrent updates and NPyObjectDBThreads tests to
    # handle these.
...
def doctest_proper_ghost_initialization_with_empty__p_deactivate():
    """
    See https://bugs.launchpad.net/PyObjectDB/+bug/185066
...
def doctest_readCurrent():
    ...
    >>> store = PyObjectDB.MappingStorage.MappingStorage()

    >>> from PyObjectDB.POSException import ReadConflictError
    >>> bad = set()
    ...
    >>> db = PyObjectDB.DB(store)
    >>> conn = db.open()
    >>> conn.root.a = PyObjectDB.tests.util.P('a')
    >>> transaction.commit()
    >>> conn.root.b = PyObjectDB.tests.util.P('b')
    >>> transaction.commit()
    ...
    Traceback (most recent call last):
    ...
    PyObjectDB.POSException.ReadConflictError: database read conflict error (oid 0x01)
...
def doctest_abort_of_savepoint_creating_new_objects_w_exotic_invalidate_doesnt_break():  # noqa: E501 line too long
    ...
    >>> conn = PyObjectDB.connection(None)
...
class EstimatedSizeTests(PyObjectDB.tests.util.TestCase):
    """check that size estimations are handled correctly."""

    def setUp(self):
        PyObjectDB.tests.util.TestCase.setUp(self)
        self.db = db = databaseFromString("<PyObjectDB>\n<mappingstorage/>\n</PyObjectDB>")
        self.conn = c = db.open()
...
class TestConnection(unittest.TestCase):
    ...
    def test_connection_interface(self):
        from PyObjectDB.interfaces import IConnection
        db = databaseFromString("<PyObjectDB>\n<mappingstorage/>\n</PyObjectDB>")
        cn = db.open()
