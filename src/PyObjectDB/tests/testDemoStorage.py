...
class DemoStorageTests(
    ...
    def setUp(self):
        ...
        self._storage = PyObjectDB.DemoStorage.DemoStorage()
    ...
    def testLoadDelegation(self):
        ...
        s2 = PyObjectDB.DemoStorage.DemoStorage(base=self._storage)
        self.assertEqual(load_current(s2, PyObjectDB.utils.z64),
                         load_current(self._storage, PyObjectDB.utils.z64))
...
class DemoStorageHexTests(DemoStorageTests):
    ...
    def setUp(self):
        ...
        self._storage = PyObjectDB.tests.hexstorage.HexStorage(
            PyObjectDB.DemoStorage.DemoStorage())
...
class DemoStorageWrappedBase(DemoStorageTests):
    ...
    def setUp(self):
        ...
        self._storage = PyObjectDB.DemoStorage.DemoStorage(base=self._base)
...
class DemoStorageWrappedAroundMappingStorage(DemoStorageWrappedBase):
    ...
    def _makeBaseStorage(self):
        from PyObjectDB.MappingStorage import MappingStorage
...
class DemoStorageWrappedAroundHexMappingStorage(DemoStorageWrappedBase):
    ...
    def _makeBaseStorage(self):
        from PyObjectDB.MappingStorage import MappingStorage
        return PyObjectDB.tests.hexstorage.HexStorage(MappingStorage())
...
def testSomeDelegation():
    ...
    >>> from PyObjectDB.DemoStorage import DemoStorage
...
def blob_pos_key_error_with_non_blob_base():
    """
    >>> storage = PyObjectDB.DemoStorage.DemoStorage()
    >>> storage.loadBlob(PyObjectDB.utils.p64(1), PyObjectDB.utils.p64(1))
    Traceback (most recent call last):
    ...
    PyObjectDB.POSException.POSKeyError: 0x01

    >>> storage.openCommittedBlobFile(PyObjectDB.utils.p64(1), PyObjectDB.utils.p64(1))
    Traceback (most recent call last):
    ...
    PyObjectDB.POSException.POSKeyError: 0x01
...
def load_before_base_storage_current():
    ...
    >>> import time
    >>> import PyObjectDB.DB
    >>> import PyObjectDB.DemoStorage
    >>> import PyObjectDB.MappingStorage
    >>> import PyObjectDB.utils

    >>> base = PyObjectDB.MappingStorage.MappingStorage()
    >>> basedb = PyObjectDB.DB(base)
    >>> conn = basedb.open()
    >>> conn.root()['foo'] = 'bar'
    >>> transaction.commit()
    >>> conn.close()
    >>> storage = PyObjectDB.DemoStorage.DemoStorage(base=base)
    ...
    >>> oid = PyObjectDB.utils.z64
    >>> base_current = load_current(storage.base, oid)
    >>> tid = PyObjectDB.utils.p64(PyObjectDB.utils.u64(base_current[1]) + 1)
...
def test_suite():
    suite = unittest.TestSuite((
        doctest.DocTestSuite(
            setUp=setUp, tearDown=PyObjectDB.tests.util.tearDown,
