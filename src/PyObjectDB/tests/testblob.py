...
import PyObjectDB.tests.IteratorStorage
import PyObjectDB.tests.StorageTestBase
import PyObjectDB.tests.util
from PyObjectDB._compat import Pickler
from PyObjectDB._compat import Unpickler
from PyObjectDB._compat import _protocol
from PyObjectDB.blob import Blob
from PyObjectDB.blob import BushyLayout
from PyObjectDB.DB import DB
from PyObjectDB.FileStorage import FileStorage
from PyObjectDB.tests.testConfig import ConfigTestBase
...
class PyObjectDBBlobConfigTest(ConfigTestBase):
...
class BlobCloneTests(PyObjectDB.tests.util.TestCase):
    ...
    def testDeepCopyCanInvalidate(self):
        ...
        http://mail.zope.org/pipermail/PyObjectDB-dev/2008-August/012054.html)
        """
        import PyObjectDB.MappingStorage
        database = DB(PyObjectDB.blob.BlobStorage(
            'blobs', PyObjectDB.MappingStorage.MappingStorage()))
...
class BushyLayoutTests(PyObjectDB.tests.util.TestCase):
...
class BlobTestBase(PyObjectDB.tests.StorageTestBase.StorageTestBase):

    def setUp(self):
        PyObjectDB.tests.StorageTestBase.StorageTestBase.setUp(self)
...
class RecoveryBlobStorage(BlobTestBase,
                          PyObjectDB.tests.IteratorStorage.IteratorDeepCompare):
    ...
    def testSimpleBlobRecovery(self):
        self.assertTrue(
            PyObjectDB.interfaces.IBlobStorageRestoreable.providedBy(self._storage)
        )
        db = DB(self._storage)
        conn = db.open()
        conn.root()[1] = PyObjectDB.blob.Blob()
        transaction.commit()
        conn.root()[2] = PyObjectDB.blob.Blob()
        with conn.root()[2].open('w') as file:
            file.write(b'some data')
        transaction.commit()
        conn.root()[3] = PyObjectDB.blob.Blob()
...
def commit_from_wrong_partition():
    ...
    >>> import logging
    >>> logger = logging.getLogger('PyObjectDB.blob.copied')
...
def test_blob_file_permissions():
    """
    >>> blob_storage = create_storage()  # noqa: F821 undefined name
    >>> conn = PyObjectDB.connection(blob_storage)
    >>> conn.root.x = PyObjectDB.blob.Blob(b'test')
...
def loadblob_tmpstore():
    ...
    >>> from PyObjectDB.Connection import TmpStore
...
def is_blob_record():
    ...
    >>> db = DB(bs)
    >>> conn = db.open()
    >>> conn.root()['blob'] = PyObjectDB.blob.Blob()
    >>> transaction.commit()
    >>> PyObjectDB.blob.is_blob_record(load_current(bs, PyObjectDB.utils.p64(0))[0])
    False
    >>> PyObjectDB.blob.is_blob_record(load_current(bs, PyObjectDB.utils.p64(1))[0])
    True

    An invalid pickle yields a false value:

    >>> PyObjectDB.blob.is_blob_record(b"Hello world!")
    False
    >>> PyObjectDB.blob.is_blob_record(b'c__main__\nC\nq\x01.')
    False
    >>> PyObjectDB.blob.is_blob_record(b'cWaaaa\nC\nq\x01.')
    False

    As does None, which may occur in delete records:

    >>> PyObjectDB.blob.is_blob_record(None)
    False
...
def savepoint_isolation():
    ...
    >>> db = DB(bs)
    >>> conn = db.open()
    >>> conn.root.b = PyObjectDB.blob.Blob(b'initial')
    >>> transaction.commit()
...
def savepoint_commits_without_invalidations_out_of_order():
    ...
    >>> conn1.root.b = PyObjectDB.blob.Blob(b'initial')
    >>> tm1.commit()
...
def lp440234_Setting__p_changed_of_a_Blob_w_no_uncomitted_changes_is_noop():
    r"""
    >>> db = PyObjectDB.DB('data.fs', blob_dir='blobs')
    >>> conn = db.open()
    >>> blob = PyObjectDB.blob.Blob(b'blah')
    >>> conn.add(blob)
    >>> transaction.commit()
...
def timeIncreasesSetUp(test):
    ...
    layer = test.globs['time_layer'] = (
        PyObjectDB.tests.util.MonotonicallyIncreasingTimeMinimalTestLayer(''))
    layer.testSetUp()
...
def setUpBlobAdaptedFileStorage(test):
    ...
    def create_storage(name='data', blob_dir=None):
        ...
        return PyObjectDB.blob.BlobStorage(blob_dir, FileStorage('%s.fs' % name))
...
def storage_reusable_suite(prefix, factory,
    ...
    suite.layer = PyObjectDB.tests.util.MonotonicallyIncreasingTimeMinimalTestLayer(
        prefix + 'BlobTests')
...
def test_suite():
    ...
    suite.addTest(loadTestsFromTestCase(PyObjectDBBlobConfigTest))
    ...
    suite.addTest(storage_reusable_suite(
        ...
        PyObjectDB.blob.BlobStorage(blob_dir, FileStorage('%s.fs' % name)),
