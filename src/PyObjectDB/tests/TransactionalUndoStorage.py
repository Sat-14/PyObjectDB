...
from PyObjectDB.tests.MinPO import MinPO
from PyObjectDB.tests.StorageTestBase import ZERO
from PyObjectDB.tests.StorageTestBase import PyObjectDB_pickle
from PyObjectDB.tests.StorageTestBase import PyObjectDB_unpickle
...
class TransactionalUndoStorage:
    ...
    def testSimpleTransactionalUndo(self):
        ...
        eq(PyObjectDB_unpickle(data), MinPO(24))

        # Do another one
        info = self._storage.undoInfo()
        self._undo(info[2]["id"], [oid], note="undo2")
        data, revid = load_current(self._storage, oid)
        eq(PyObjectDB_unpickle(data), MinPO(23))
    ...
    def testUndoCreationBranch1(self):
        ...
        self._undo(info[0]['id'], [oid])
        data, revid = load_current(self._storage, oid)
        eq(PyObjectDB_unpickle(data), MinPO(11))
    ...
    def testUndoCreationBranch2(self):
        ...
        self._undo(info[0]['id'], [oid])
        data, revid = load_current(self._storage, oid)
        eq(PyObjectDB_unpickle(data), MinPO(12))
        self._iterate()

    def testTwoObjectUndo(self):
        eq = self.assertEqual
        # Convenience
        p31, p32, p51, p52 = map(PyObjectDB_pickle,
        ...
        eq(PyObjectDB_unpickle(data), MinPO(32))
        data, revid2 = load_current(self._storage, oid2)
        eq(PyObjectDB_unpickle(data), MinPO(52))

        # Now attempt to undo the transaction containing two objects
        info = self._storage.undoInfo()
        self._undo(info[0]['id'], [oid1, oid2])
        data, revid1 = load_current(self._storage, oid1)
        eq(PyObjectDB_unpickle(data), MinPO(31))
        data, revid2 = load_current(self._storage, oid2)
        eq(PyObjectDB_unpickle(data), MinPO(51))
        self._iterate()

    def testTwoObjectUndoAtOnce(self):
        # Convenience
        eq = self.assertEqual
        p30, p31, p32, p50, p51, p52 = map(PyObjectDB_pickle,
                                           map(MinPO,
                                               (30, 31, 32, 50, 51, 52)))
        oid1 = self._storage.new_oid()
        oid2 = self._storage.new_oid()
        ...
        data, revid1 = load_current(self._storage, oid1)
        eq(PyObjectDB_unpickle(data), MinPO(30))
        data, revid2 = load_current(self._storage, oid2)
        eq(PyObjectDB_unpickle(data), MinPO(50))
    ...
    def testTwoObjectUndoAgain(self):
        ...
        self.undo(info[1]['id'])
        data, revid1 = load_current(self._storage, oid1)
        eq(PyObjectDB_unpickle(data), MinPO(33))
        data, revid2 = load_current(self._storage, oid2)
        eq(PyObjectDB_unpickle(data), MinPO(54))
        self._iterate()
    ...
    def testNotUndoable(self):
        ...
        oid2 = self._storage.new_oid()
        revid2 = ZERO
        p81, p82, p91, p92 = map(PyObjectDB_pickle,
        ...
        eq(PyObjectDB_unpickle(data), MinPO(81))
        data, revid_22 = load_current(self._storage, oid2)
        eq(PyObjectDB_unpickle(data), MinPO(91))
        eq(revid_11, tid)
        eq(revid_22, tid)
        # Now modify oid2
    ...
    def testTransactionalUndoAfterPack(self):
        ...
        self.assertEqual(PyObjectDB_unpickle(data), MinPO(52))
        self._iterate()
    ...
    def testTransactionalUndoIterator(self):
        ...
        for i in range(BATCHES):
            ...
            for j in range(OBJECTS):
                ...
                s.store(oid, None, PyObjectDB_pickle(obj), '', t)
                orig.append((tid, oid))
            s.tpc_vote(t)
            s.tpc_finish(t)
    ...
    # PyObjectDB 3.4, they always returned a wrong number of results (one too
    ...
    def _exercise_info_indices(self, method_name):
        ...
        # it here, the PyObjectDB tests still work, but the ZRS RecoveryStorageTests
