...
def PyObjectDB_pickle(obj):
    """Create a pickle in the format expected by PyObjectDB."""
    f = BytesIO()
...
def persistent_load(pid):
    # helper for PyObjectDB_unpickle
    return f"ref to {pid[1][0]}.{pid[1][1]} oid={u64(pid[0])}"


def PyObjectDB_unpickle(data):
    """Unpickle an object stored using the format expected by PyObjectDB."""
    f = BytesIO(data)
    u = Unpickler(f)
...
class StorageTestBase(PyObjectDB.tests.util.TestCase):
