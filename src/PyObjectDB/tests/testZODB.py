...
from PyObjectDB.POSException import TransactionFailedError
...
class PyObjectDBTests(PyObjectDB.tests.util.TestCase):
    ...
    def testResetCachesAPI(self):
        ...
        conn.root()
        self.assertGreater(len(conn._cache), 0)  # Precondition
        PyObjectDB.Connection.resetCaches()
        conn.close()
    ...
    def testSavepointDoesntGetInvalidations(self):
        # Prior to PyObjectDB 3.2.9 and 3.4, Connection.tpc_finish() processed
        ...
        # Prior to PyObjectDB 3.2.9 and 3.4, q.value was 5 here.
        rt = conn.root()
        try:
            self.assertEqual(rt["p"].value, 3)
            self.assertEqual(rt["q"].value, 2)
    ...
    def testTxnBeginImpliesAbort(self):
        ...
        # down failed).
        # Later (PyObjectDB 3.6):  Transaction.begin() no longer exists, so the
    ...
    def testMultipleUndoInOneTransaction(self):
        ...
        # operations within a transaction.  If PyObjectDB performs the undo
