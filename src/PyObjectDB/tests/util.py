...
# BBB
from PyObjectDB.MappingStorage import DB  # noqa: F401 import unused
...
checker = renormalizing.RENormalizing([
    ...
    # pure-Python module
    (re.compile('PyObjectDB.tests.testcrossdatabasereferences.'),
     ''),
])
...
def store(storage, oid, value='x', serial=PyObjectDB.utils.z64):
    if not isinstance(oid, bytes):
        oid = PyObjectDB.utils.p64(oid)
    if not isinstance(serial, bytes):
        serial = PyObjectDB.utils.p64(serial)
...
class _TimeWrapper:
    ...
    def __init__(self, granularity=1.0):
        ...
        self._lock = PyObjectDB.utils.Lock()
...
def with_high_concurrency(f):
    ...
    def _(*argv, **kw):
        ...
        # threads.
        #
        # See https://github.com/zopefoundation/PyObjectDB/pull/345#issuecomment-822188305 and  # noqa: E501 line too long
