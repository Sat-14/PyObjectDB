...
class CacheErrors(unittest.TestCase):
    ...
    def testTwoObjsSameOid(self):
        ...
        # PyObjectDB 3.2.6 didn't make sense.  This test verifies that (a) an
...
def test_basic_cache_size_estimation():
    ...
    >>> import PyObjectDB.MappingStorage
    >>> db = PyObjectDB.MappingStorage.DB()
    >>> conn = db.open()
...
We add some data and the cache grows:
    ...
    >>> conn.root.z = PyObjectDB.tests.util.P('x'*100)
    >>> import transaction
    >>> transaction.commit()
    >>> root_size = conn.root._root._p_estimated_size
