    ...
    def iter_verify(self, txniter, revids, val0):
        ...
        for reciter, revid in zip(txniter, revids + [None]):
            ...
            for rec in reciter:
                eq(rec.oid, oid)
                eq(rec.tid, revid)
                eq(PyObjectDB_unpickle(rec.data), MinPO(val))
                val = val + 1
...
class IteratorStorage(IteratorCompare):
    ...
    def testIterationIntraTransaction(self):
        ...
        # like
        #
        # PyObjectDB FS FS21 warn: FileStorageTests.fs truncated, possibly due to
        ...
        oid = self._storage.new_oid()
        t = TransactionMetaData()
        data = PyObjectDB_pickle(MinPO(0))
        try:
            self._storage.tpc_begin(t)
    ...
    def testLoad_was_checkLoadEx(self):
        ...
        self.assertEqual(PyObjectDB_unpickle(data), MinPO(42))
        match = False
