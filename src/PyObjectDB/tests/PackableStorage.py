...
from PyObjectDB.tests.MTStorage import TestThread
from PyObjectDB.tests.StorageTestBase import snooze
from PyObjectDB.tests.util import time_monotonically_increases
...
class PackableStorageBase:
    ...
    def _sanity_check(self):
        ...
        if not PyObjectDB.interfaces.IStorageIteration.providedBy(self._storage):
            return
...
class PackableStorage(PackableStorageBase):
    ...
    def testPackWithMultiDatabaseReferences(self):
        ...
        otherdb = PyObjectDB.tests.util.DB(databases=databases, database_name='o')
        conn = db.open()
        root = conn.root()
        root[1] = C()
        transaction.commit()
