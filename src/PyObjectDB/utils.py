...
# Given a PyObjectDB pickle, return pair of strings (module_name, class_name).
# Do this without importing the module or class object.
# See PyObjectDB/serialize.py's module docstring for the only docs that exist about
# PyObjectDB pickle format.  If the code here gets smarter, please update those
...
def get_pickle_metadata(data):
    ...
    # PyObjectDB's data records contain two pickles.  The first is the class
...
if os.environ.get('DEBUG_LOCKING'):  # pragma: no cover
    class Lock:
        ...
        def pr(self, name, a=None, kw=None):
            f = sys._getframe(2)
            if f.f_code.co_filename.endswith('PyObjectDB/utils.py'):
                f = sys._getframe(3)
...
import PyObjectDB.POSException  # noqa: E402 module level import not at top of file


def load_current(storage, oid, version=''):
    """Load the most recent revision of an object by calling loadBefore

    Starting in PyObjectDB 5, it's no longer necessary for storages to
