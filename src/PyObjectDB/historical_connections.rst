...
Moreover, the historical connection cannot commit changes.
    ...
    ...
    PyObjectDB.POSException.ReadOnlyHistoryError
    >>> transaction1.abort()
    >>> historical_conn.root()['first']['count']
    0
...
All three of these values can be specified in a ZConfig file.
    ...
    >>> db2 = PyObjectDB.config.databaseFromString('''
    ...     <PyObjectDB>
    ...       <mappingstorage/>
...
somewhat naively in the PyObjectDB (largely because they are handled naively in Unix/
POSIX time) so any minute that contains a leap second may contain serials that
are a bit off. This is not generally a problem for the PyObjectDB, because serials
