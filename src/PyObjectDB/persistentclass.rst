...
Let's look at an example:
    ...
    >>> C = PyObjectDB.persistentclass.PersistentMetaClass(
    ...     'C', (object, ), dict(
    ...     __init__ = __init__,
    ...     __module__ = '__PyObjectDB__',
    ...     foo = foo,
    ...     kind = 'sample',
    ...     ))
...
Now, if we try to load it, we get a broken object:
    ...
    <persistent broken __PyObjectDB__.P instance b'\x00\x00\x00\x00\x00\x00\x00\x04'>

because the module, `__PyObjectDB__` can't be loaded.  We need to provide a
...
if the module is `__PyObjectDB__`.  It falls back to the normal class lookup
for other modules:

    >>> from PyObjectDB.broken import find_global
    >>> def classFactory(connection, modulename, globalname):
    ...     if modulename == '__PyObjectDB__':
