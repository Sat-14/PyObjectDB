...
@zope.interface.implementer(PyObjectDB.interfaces.IBroken)
class Broken:
    """Broken object base class
       ...
       You can't modify broken objects::
         ...
         ...
         PyObjectDB.broken.BrokenModified: Can't change broken objects
...
def find_global(modulename, globalname,
    ...
    """Find a global object, returning a broken class if it can't be found.
       ...
       If an object can't be found, a broken class is returned::
         ...
         >>> broken = find_global('PyObjectDB.not.there', 'atall')
         >>> issubclass(broken, Broken)
         True
         >>> broken.__module__
         'PyObjectDB.not.there'
         >>> broken.__name__
         'atall'

       Broken classes are cached::

         >>> find_global('PyObjectDB.not.there', 'atall') is broken
         True

       If we "repair" a missing global::

         >>> class PyObjectDBnotthere(object):
         ...     atall = []

         >>> sys.modules['PyObjectDB.not'] = PyObjectDBnotthere
         >>> sys.modules['PyObjectDB.not.there'] = PyObjectDBnotthere

       we can then get the repaired value::

         >>> find_global('PyObjectDB.not.there', 'atall') is PyObjectDBnotthere.atall
         True
...
def rebuild(modulename, globalname, *args):
    """Recreate a broken object, possibly recreating the missing class
       ...
       This functions unpickles broken objects::
         ...
         >>> broken = rebuild('PyObjectDB.notthere', 'atall', 1, 2)
         >>> broken
         <broken PyObjectDB.notthere.atall instance>
       ...
       If we "repair" the brokenness::
         ...
         >>> sys.modules['PyObjectDB.notthere'] = notthere

         >>> rebuild('PyObjectDB.notthere', 'atall', 1, 2)
         atall 1 2

         >>> del sys.modules['PyObjectDB.notthere']
...
class PersistentBroken(Broken, persistent.Persistent):
    r"""Persistent broken objects
        ...
        and persistent broken objects aren't directly picklable:
          ...
          Traceback (most recent call last):
          ...
          PyObjectDB.broken.BrokenModified:
          <persistent broken not.there.Atall instance '\x00\x00\x00\x00****'>
