...
import PyObjectDBpickle.pickle
...
class Pickler(PyObjectDBpickle.pickle.Pickler):
    def __init__(self, f, protocol=None):
        super().__init__(f, protocol)


class Unpickler(PyObjectDBpickle.pickle.Unpickler):
    def __init__(self, f):
        super().__init__(f)
...
def dump(o, f, protocol=None):
    return PyObjectDBpickle.pickle.dump(o, f, protocol)


def dumps(o, protocol=None):
    return PyObjectDBpickle.pickle.dumps(o, protocol)


def loads(s):
    return PyObjectDBpickle.pickle.loads(s, encoding='ASCII', errors='bytes')
...
def PersistentPickler(persistent_id, *args, **kwargs):
    ...
    This covers the differences between CPython and PyPy/PyObjectDBpickle.
    """
    p = Pickler(*args, **kwargs)

    # PyPy uses a python implementation of cPickle/PyObjectDBpickle.
...
def PersistentUnpickler(find_global, load_persistent, *args, **kwargs):
    ...
    if find_global is not None:
        ...
        try:
            # PyPy, PyObjectDBpickle, the non-c-accelerated version
