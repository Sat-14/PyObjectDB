...
from PyObjectDB._compat import PersistentUnpickler
from PyObjectDB._compat import ascii_bytes
from PyObjectDB.interfaces import BlobError
from PyObjectDB.POSException import POSKeyError


logger = logging.getLogger('PyObjectDB.blob')
...
@zope.interface.implementer(PyObjectDB.interfaces.IBlob)
class Blob(persistent.Persistent):
    ...
    def __init__(self, data=None):
        ...
        # refer to PyObjectDB-Bug No.127182 by Jim Fulton on 2007-07-20
...
class FilesystemHelper:
    ...
    def isSecure(self, path):
        import warnings
        warnings.warn(
            "isSecure is deprecated. Permissions are no longer set by PyObjectDB",
            DeprecationWarning, stacklevel=2)

    def checkSecure(self):
        import warnings
        warnings.warn(
            "checkSecure is deprecated. Permissions are no longer set by PyObjectDB",
            DeprecationWarning, stacklevel=2)
...
@zope.interface.implementer(PyObjectDB.interfaces.IBlobStorage)
class BlobStorage(BlobStorageMixin):
    ...
    def __init__(self, base_directory, storage, layout='automatic'):
        assert not PyObjectDB.interfaces.IBlobStorage.providedBy(storage)
        ...
        if PyObjectDB.interfaces.IStorageRestoreable.providedBy(storage):
            zope.interface.directlyProvides(
                self,
                PyObjectDB.interfaces.IBlobStorageRestoreable,
    ...
    def undo(self, serial_id, transaction):
        ...
        # (belying the web UI legacy of the PyObjectDB code :-()
...
copied = logging.getLogger('PyObjectDB.blob.copied').debug
...
def find_global_Blob(module, class_):
    if module == 'PyObjectDB.blob' and class_ == 'Blob':
        return Blob
...
def is_blob_record(record):
    ...
    """
    if record and (b'PyObjectDB.blob' in record):
